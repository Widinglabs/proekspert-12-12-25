...
class Product(BaseModel):
    ...
    product_id: int = Field(
        ..., description="Unique product identifier", gt=0, examples=[1, 42, 1337]
    )
    ...
    product_in_stock: bool = Field(
        default=True, description="Whether the product is currently available for purchase"
    )
...
class ProductListResponse(BaseModel):
    ...
    products: list[Product] = Field(
        ..., description="List of products matching the request criteria"
    )
